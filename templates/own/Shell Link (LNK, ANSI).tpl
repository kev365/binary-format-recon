template "Shell Link (LNK, ANSI)"
description "Windows Shell Link .lnk, system-code-page StringData - MS-SHLLINK v20251121"
...
//
// Use this variant when LinkFlags bit 7 (IsUnicode) is clear. When it is set,
// use "Shell Link (LNK, Unicode)": StringData is then UTF-16 at two bytes per
// character.
//
...
IfEqual HasName 1
    uint16 NameChars                         // characters, one byte each here
    string NameChars "NAME_STRING (description)"
EndIf
IfEqual HasRelativePath 1
    uint16 RelPathChars
    string RelPathChars "RELATIVE_PATH"
EndIf
IfEqual HasWorkingDir 1
    uint16 WorkDirChars
    string WorkDirChars "WORKING_DIR"
EndIf
IfEqual HasArguments 1
    uint16 ArgsChars
    string ArgsChars "COMMAND_LINE_ARGUMENTS"
EndIf
IfEqual HasIconLocation 1
    uint16 IconChars
    string IconChars "ICON_LOCATION"
EndIf
endsection
